...
# 1b Readin data 
# 1b.i Start with empty dta
dta <- fst::read_fst(paste0(intermediate.data.folder, "c_4_casecontrolhours_assigned_variability_",
                           CaseType, "_", YYYY.list[1], "_", NLDASWeight, "_", Timing , ".fst"))

# 1b.ii Fill with data

for(i in 2:length(YYYY.list)){
    dta <- dta %>% 
      bind_rows(fst::read_fst(paste0(intermediate.data.folder,
                                     "c_4_casecontrolhours_assigned_variability_", CaseType,"_", 
                                    YYYY.list[i],"_", NLDASWeight, "_", Timing, ".fst")))
}
...
if(Timing != "All23"){
  Miss <- read_csv(paste0(final.data.folder, "PercentMissing_", CaseType, ".csv"))
  Miss$NLDASErr <- nrow(dtaA) - nrow(dta)
  Miss %>% write_csv(paste0(final.data.folder, "PercentMissing_", CaseType, ".csv"))
}
####**************************************
#### 3: Create Data-Derived Variables ####
####**************************************
  ...
  dta <- dta %>% 
    mutate(
      StrokeSubType  = case_when(
        ...
        PR01 == "61645" | PR02 == "61645" | PR03 =="61645" | PR04 =="61645" | PR05 == "61645" ~ "isc",
    ...
    mutate(strokeISC_prim = if_else(isc_stroke_prim==1&(StrokeSubType=="isc"),1,0),
           strokeHEM_prim = if_else(hem_stroke_prim==1&(StrokeSubType=="hem"),1,0))
}
...
dta %>% 
  ...
  write_csv(paste0(final.data.folder, "HHFreq", "_", CaseType, "_", Timing, ".csv"))
...
if(CaseType == "mi"){
  dta <- dta %>% 
    dplyr::select(contains("prim"), EventID, HourName0,# Case features / strata identifiers
                ...
                sex, race,  ageGroup, wait, First, MIStatus, 
                contains("DX"), contains("pr"), STEMI, CaseDateRawOrig)
}
if(CaseType == "stroke"){
  dta <- dta %>% 
    dplyr::select(contains("prim"), StrokeSubType, EventID, HourName0,# Case features / strata identifiers
                  ...
                  contains("24"), contains("48"), contains("72"), contains("36"),
                  CaseDateRaw, CaseHourIndex, UPID, zcta, 
                  sex, race, htn, ageGroup, wait, First, 
                  contains("DX"), contains("pr"), CaseDateRawOrig)
}
...
if(CaseType =="stroke"){
  dta.deid <- dta.deid %>% 
    mutate(stroke_prim = if_else(isc_stroke_prim==1 | hem_stroke_prim==1, 1, 0)) %>%
