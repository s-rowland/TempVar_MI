...
raw.grib.folder.2016 <- "~/Desktop/ExtraSteps_Temp/Data/Raw Data/NLDAS_2016"
...
tempL %>% spread(TimeHour, temp) %>% 
  fst::write_fst(paste0(NLDAS.data.folder, "2016_TMP", "_NLDAS_NY.fst")) 
spfhL %>% spread(TimeHour, spfh) %>% 
  fst::write_fst(paste0(NLDAS.data.folder, "2016_SPFH", "_NLDAS_NY.fst")) 
presL %>% spread(TimeHour, pres) %>% 
  fst::write_fst(paste0(NLDAS.data.folder, "2016_PRES", "_NLDAS_NY.fst")) 
