...
plot_TempVar_ER <- function(CaseType, TempVarMetric,  ERConstraint, SubPopVar, SubPop, Sensitivity, TempVarRange){
 ...
 Exposure_vector <- Exposure_vector %>% mutate(Info = "Density")
  ...
  if(TempVarMetric !="DayDiffT_48hr"){YMin <- -4; YMax <- 4}
  ...
  #if(TempVarMetric == "MeanAbsFDT_24hr"){Letter1 <- "A"; YMaxDensity <- 1.3; Letter2 <- "D"}
  #if(TempVarMetric == "MeanFDT_24hr"){Letter1 <- "B"; YMaxDensity <- 1.3; Letter2 <- "D"}
  est.table[(nrow(est.table)+1),] <- list(est.table$TempVar[VAL], est.table$fit.pc[VAL], 
                                            est.table$lci.pc[VAL], est.table$uci.pc[VAL], 
                                            "Change in \n MI Hospitalizations (%)", NA, YMax, " ")
  est.table[(nrow(est.table)+1),] <- list(est.table$TempVar[VAL], est.table$fit.pc[VAL], 
                                            est.table$lci.pc[VAL], est.table$uci.pc[VAL], 
                                            "Change in \n MI Hospitalizations (%)", NA, -4, "a")
  ...
  TP <- ggplot(est.table) + 
    ...
    #geom_vline(aes(xintercept = TempVar025th), color = "grey42") + 
    #geom_vline(aes(xintercept = TempVar975th), color = "grey42") + 
    ...
    xlim(TempVar025th, TempVar975th)+
    labs(y = paste0(" ")) +
  ...
  # 1C.h Save image
  #tiff(paste0(manuscript.folder, "Fig2_ER_", TempVarMetric, ".tiff"), 
   #    height = HH.fig, width = WW.fig, res = RR.fig)
  if(TempVarMetric == "MeanAbsFDT_24hr"){LetterSet <- c("A", "C")}
  if(TempVarMetric == "MeanFDT_24hr"){LetterSet <- c("B", "D")}
  
  print(tag_facet(TP, open ="", close = "", tag_pool = LetterSet, size = 10))
  #print(TP)
  #dev.off()
...
# 2a Create plots
TP.a <- plot_TempVar_ER("mi", "MeanAbsFDT_24hr", "3dfevenknots", "fullpop", "fullpop", "main", "025to975")
TP.b <- plot_TempVar_ER("mi", "MeanFDT_24hr", "4dfevenknots", "fullpop", "fullpop", "main", "025to975")
tiff(paste0(manuscript.folder, "Fig2_ER_partialExpRange.tiff"), 
    height = HH.fig, width = WW.fig*1.25, res = RR.fig)
grid.arrange(TP.a, TP.b, nrow=1)
dev.off()
