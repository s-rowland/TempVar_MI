...
plot_TempVar_ER <- function(CaseType, TempVarMetric,  ERConstraint, SubPopVar, SubPop, Sensitivity, TempVarRange){
  ...
  # 1C.a Create the string of the metric name, for the plot label
  ...
  if(TempVarMetric == "MeanAbsFDT_24hr"){VAL <- 35}
  if(TempVarMetric == "MeanFDT_24hr"){VAL <- 10}
  est.table[(nrow(est.table)+1),] <- list(est.table$TempVar[VAL], est.table$fit.pc[VAL], 
                                            est.table$lci.pc[VAL], est.table$uci.pc[VAL], 
                                            "Change in \n MI Hospitalizations (%)", NA, 15, TempVarMetricName)
  est.table[(nrow(est.table)+1),] <- list(est.table$TempVar[VAL], est.table$fit.pc[VAL], 
                                            est.table$lci.pc[VAL], est.table$uci.pc[VAL], 
  ...
  if(TempVarMetric %in% NamesAbsChangeOverTimeExpW){
    ...
    ActiveColor <- ColBagOfValues; ActiveColorCI <- ColBagOfValuesCI
  ...
  TP <- ggplot(est.table) + 
    ...
    geom_text(aes(x=TempVar,y=YMaxValue, label=LabelValue), size = 6)+
  ...
  # 1C.h Save plot
 #tiff(paste0(manuscript.folder, "eFig3_ER_", TempVarMetric, ".tiff"), 
...
# 2a Create plots
TP.a <- plot_TempVar_ER("mi", "MeanAbsFDT_24hr", "3dfevenknots", "fullpop", "fullpop", "main", "00to100")
TP.b <- plot_TempVar_ER("mi", "MeanFDT_24hr", "3dfevenknots", "fullpop", "fullpop", "main", "00to100")

tiff(paste0(manuscript.folder, "eFig3_ER_FullExpRange.tiff"), 
     height = HH.fig, width = 1.5 *WW.fig, res = RR.fig)
